...
export function mountSiteHeader(): () => void {
  ...
  function navScroll() {
    ...
    // Both layouts can hold a [data-nav-hero] at once (one hidden by CSS), and
    // a hidden ancestor reads offsetHeight 0, so pick the rendered one.
    ...
    const menuOpen = menuEl?.getAttribute("data-open") === "1";
    const y = window.scrollY;
    const solid = y > hero.offsetHeight - 90;

    // Cheap state first: bail out before any hit-testing or luminance work.
    const coarse = `${solid ? "s" : "t"}${menuOpen ? "m" : ""}`;
    const toneStale = Math.abs(y - toneY) > 160;
    if (nav.dataset.navcoarse === coarse && !toneStale) return;
    nav.dataset.navcoarse = coarse;

    if (toneStale) {
      // One probe, at the logo's own centre. A section can declare its tone
      // with [data-nav-tone] (the sampler cannot read video pixels), and the
      // nav sits over this pixel, so walk the hit stack past it.
      const logo = document.getElementById("sg-logo-link");
      const r = logo ? logo.getBoundingClientRect() : null;
      const px = Math.round(r ? r.left + r.width * 0.5 : window.innerWidth * 0.5);
      const py = Math.round(r ? r.top + r.height * 0.5 : 74);
      let tone: string | null = null;
      const stack = Array.from(document.elementsFromPoint(px, py));
      for (const el of stack) {
        if (nav.contains(el)) continue;
        const declared = el.closest("[data-nav-tone]");
    ...
    // Marks switch to their inverted treatment once the nav floats over content.
    nav.dataset.invert = solid && !menuOpen ? "1" : "0";

    const darkBars = !menuOpen && !darkBg;
    nav.querySelectorAll<HTMLElement>(".sg-bar").forEach((bar) => {
      bar.style.background = darkBars ? "#141414" : "#fff";
    });
  }
